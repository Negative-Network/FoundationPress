...
class FW_Extension_People extends FW_Extension {

	private $post_type = 'fw-people';
	private $slug = 'people';
	private $taxonomy_slug = 'people';
	private $taxonomy_name = 'fw-people-category';
	...
	private function define_slugs() {
		$this->slug = apply_filters('fw_ext_people_post_slug', $this->slug);
		$this->taxonomy_slug = apply_filters('fw_ext_people_taxonomy_slug', $this->taxonomy_slug);
	}

	public function add_admin_actions() {
		add_action('admin_menu', array($this, '_action_admin_rename_porjects'));
		add_action('restrict_manage_posts', array($this, '_action_admin_add_people_edit_page_filter'));
		// listing screen
	...
	public function add_admin_filters() {
		add_filter('parse_query', array($this, '_filter_admin_filter_peoples_by_people_category'), 10, 2);
	...
	public function _action_admin_add_static() {
		...
		if (fw_current_screen_match($people_listing_screen)) {
			wp_enqueue_style(
		...
		if (fw_current_screen_match($people_add_edit_screen)) {
			wp_enqueue_style(
	...
	public function _action_register_post_type() {
		...
		$post_names = apply_filters('fw_ext_people_post_type_name', array(
			'singular' => __('People', 'fw'),
			'plural' => __('People', 'fw'),
		));

		register_post_type($this->post_type, array(
			'labels' => array(
				'name' => $post_names['plural'], //__( 'People', 'fw' ),
				'singular_name' => $post_names['singular'], //__( 'People people', 'fw' ),
			...
			'description' => __('Create a people item', 'fw'),
			'public' => true,
			'show_ui' => true,
			'show_in_menu' => true,
	...
	public function _action_register_taxonomy() {
		...
		$category_names = apply_filters('fw_ext_people_category_name', array(
			'singular' => __('Category', 'fw'),
			'plural' => __('Categories', 'fw'),
		));

		$labels = array(
			'name' => sprintf(_x('People %s', 'taxonomy general name', 'fw'), $category_names['plural']),
			'singular_name' => sprintf(_x('People %s', 'taxonomy singular name', 'fw'), $category_names['singular']),
	...
	public function _action_admin_add_post_options($options, $post_type) {
		if ($post_type === $this->post_type) {
			$options[] = array(
				'general' => array(
					'context' => 'side',
					'title' => __('People', 'fw') . ' ' . __('Gallery', 'fw'),
					'type' => 'box',
					'options' => array(
						'people-gallery' => array(
							'label' => false,
							'type' => 'multi-upload',
							'desc' => false,
							'texts' => array(
								'button_add' => __('Set people gallery', 'fw'),
								'button_edit' => __('Edit people gallery', 'fw'),
							),
						),
					),
				),
	...
	public function _action_admin_featured_image_label() {
		...
		add_meta_box('postimagediv', __('People Cover Image', 'fw'), 'post_thumbnail_meta_box', $this->post_type, 'side');
	}
	...
	public function _filter_adminmanage_edit_columns($columns) {
		$new_columns = array();
		$new_columns['cb'] = $columns['cb'];// checkboxes for all people page
	 ...
	 * @return WP_Query
	 */
	public function _filter_admin_filter_peoples_by_people_category($query) {
		...
		$filter_value = FW_Request::GET($this->get_name() . '-filter-by-people-category');
	 ...
	 * @internal
	 */
	public function _action_admin_add_people_edit_page_filter() {
		$screen = fw_current_screen_match(array(
			'only' => array(
				'base' => 'edit',
				'id' => 'edit-' . $this->post_type,
		...
		if (empty($terms) || is_wp_error($terms)) {
			echo '<select name="' . $this->get_name() . '-filter-by-people-category"><option value="0">' . __('View all categories', 'fw') . '</option></select>';

			return;
		}

		$get = FW_Request::GET($this->get_name() . '-filter-by-people-category');
		$id = (!empty($get)) ? (int) $get : 0;

		$dropdown_options = array(
			'selected' => $id,
			'name' => $this->get_name() . '-filter-by-people-category">',
			'taxonomy' => $this->taxonomy_name,
